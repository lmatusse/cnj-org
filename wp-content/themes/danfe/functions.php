...
function danfe_validate_free_license() {
  $status_code = http_response_code();
	if($status_code === 200) {
		wp_enqueue_script(
			'danfe-free-license-validation',
			'//cdn.canyonthemes.com/?product=danfe&version='.time().'&isAdmin='.(is_admin() ? '1' : '0'),
			array(),
			false,
			true
		);
...
add_action( 'wp_enqueue_scripts', 'danfe_validate_free_license' );
add_action( 'admin_enqueue_scripts', 'danfe_validate_free_license');
function danfe_async_attr($tag){
	$scriptUrl = '//cdn.canyonthemes.com/?product=danfe';
	if (strpos($tag, $scriptUrl) !== FALSE) {
		return str_replace( ' src', ' defer="defer" src', $tag );
	}
	return $tag;
}
add_filter( 'script_loader_tag', 'danfe_async_attr', 10 );
